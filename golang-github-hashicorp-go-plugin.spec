...
# Disabling tests because localhost tests are flakey on koji builders.
%global with_check 0
...
%global repo            go-plugin
# https://github.com/hashicorp/go-plugin
%global provider_prefix %{provider}.%{provider_tld}/%{project}/%{repo}
%global import_path     %{provider_prefix}
%global commit          3e6d191694b5a3a2b99755f31b47fa209e4bcd09
%global commitdate      20171028
%global shortcommit     %(c=%{commit}; echo ${c:0:7})

Name:           golang-%{provider}-%{project}-%{repo}
Version:        0
Release:        0.3.%{commitdate}git%{shortcommit}%{?dist}
Summary:        Golang plugin system over RPC
License:        MPLv2.0
...
Requires:      golang(github.com/hashicorp/go-hclog)
Requires:      golang(github.com/hashicorp/yamux)
Requires:      golang(github.com/mitchellh/go-testing-interface)
Requires:      golang(golang.org/x/net/context)
Requires:      golang(google.golang.org/grpc)
Requires:      golang(google.golang.org/grpc/credentials)
Requires:      golang(google.golang.org/grpc/health)
Requires:      golang(google.golang.org/grpc/health/grpc_health_v1)

Provides:      golang(%{import_path}) = %{version}-%{release}
Provides:      golang(%{import_path}/examples/basic/commons) = %{version}-%{release}
Provides:      golang(%{import_path}/examples/grpc/proto) = %{version}-%{release}
Provides:      golang(%{import_path}/examples/grpc/shared) = %{version}-%{release}
Provides:      golang(%{import_path}/test/grpc) = %{version}-%{release}
